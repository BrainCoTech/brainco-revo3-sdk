...
class Revo3MotorConfigPanel(QWidget):
    ...
    def _on_turbo_changed(self, state):
        if state == Qt.Checked:
            self.turbo_check.blockSignals(True)
            self.turbo_check.setChecked(False)
            self.turbo_check.blockSignals(False)
        print("[Config] Turbo mode API is not available in this Revo3-only build")

    def _apply_turbo_config(self):
        print("[Config] Turbo config API is not available in this Revo3-only build")
