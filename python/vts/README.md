...
本说明针对 Revo3 视触觉模组的数据采集与力估计权重加载。该能力依赖
独立的平台运行时，不属于 `bc-revo3-sdk` Python 包的默认依赖。

---

## 安装视触觉平台运行时

使用仓库安装脚本按操作系统和架构（Linux x86_64、Linux aarch64 或
Windows amd64）安装匹配的视触觉平台运行时：
...
安装脚本是版本和下载地址的单一入口。不要在示例文档中固定平台 wheel 地址。
...
SDK 提供了一个可以独立于主 SDK 界面运行的 VisionTouch 图形化调试面板：`python/gui/vision_touch_window.py`。该面板支持多传感器并发连接、下拉切换、实时 3D 点云/深度图渲染、以及多传感器实时六维力状态概览。
...
python -m python.gui.vision_touch_window

# 2. 自动加载力模型（指定权重目录，当模型匹配时激活力估计 Force6D 渲染）
python -m python.gui.vision_touch_window --force-model-dir python/vts/checkpoints --force-model-mode auto

# 3. 强校验力模型启动（若某些已发现的 VTS 设备缺乏对应权重，则初始化报错跳过）
python -m python.gui.vision_touch_window --force-model-dir python/vts/checkpoints --force-model-mode required
```
...
主 Python GUI (`python/gui/main.py`) 会根据 `UltraVisionTouch` 硬件类型显示 `VisionTouch` Tab。如果同一只手还上报 `mt_*` 压阻阵列或 `mx_*` 高密矩阵触觉，普通 Revo3 触觉 Tab 会同时保留。该 Tab 默认使用快速初始化模式，不加载力估计模型。
...
# 强制要求每个 VTS 都有模型
python python/gui/main.py --vts-force-model-dir python/vts/checkpoints --vts-force-model-mode required
```
