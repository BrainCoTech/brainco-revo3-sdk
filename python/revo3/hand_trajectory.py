...
async def main():
    ...
    try:
        ...
        await client.revo3_move_joint_wait(slave_id, 3, 30.0, 1.5, 0.01)

        print("Single joint: J3 -> 0 deg at 25 rpm")
        await client.revo3_move_joint_with_speed_wait(slave_id, 3, 0.0, 25.0, 0.01)

        targets = [0.0] * 21
        for joint in (2, 6, 10, 14, 18):
            targets[joint] = 45.0
        print("Full hand: PIP joints -> 45 deg with custom gains")
        await client.revo3_move_hand_with_gains_wait(slave_id, targets, 2.0, 0.01, 5.0, 0.5)

        print("Full hand: all joints -> 0 deg at 25 rpm")
        await client.revo3_move_hand_with_speed_wait(slave_id, [0.0] * 21, 25.0, 0.01)
        return 0
    finally:
