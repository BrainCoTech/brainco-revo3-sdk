...
当 `debounce_time_ms` 大于零时，检测使用时间窗口，并要求窗口内存在多次越界采样。值为零时，单次越界即可触发保护。实际响应时间仍取决于状态读取频率和总线延迟。
...
PySide GUI 保留现有碰撞测试控件和样式，并通过内部适配层调用 2.0 `hand.experimental_collision` API。GUI 本地黄色 Stall guard 是交互拖动测试的显示与控制兜底，与 SDK 确认的红色实验性碰撞状态不是同一状态源。

GUI 监控使用 2.0 State 读取和应用自行维护的显示状态，不会恢复已经移除的 Python collector 或 shared-buffer API。
...
在形成上述真机记录前，只能将该能力描述为“已实现、等待真机验证”，不得声称其能够保证停止或防止硬件损坏。
